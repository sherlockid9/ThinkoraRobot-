...
def build_qa_prompt(question: str) -> str:
    return (
        ...
        "২. TABLE (real GFM markdown table বানাতে হবে, লিখে দিলে হবে না):\n"
        "   - উত্তরে যদি কোনো তুলনা, তালিকা-ভিত্তিক সংখ্যা, ক্যাটাগরি-অনুযায়ী তথ্য, ধাপ-ভিত্তিক "
        "ডেটা, বা একাধিক আইটেমের বৈশিষ্ট্য থাকে (যেমন: orbital-এর max electron capacity, "
        "সূত্রের তালিকা, উপাদানের তুলনা, ধাপে ধাপে মান) — সেগুলো bullet list না করে "
        "অবশ্যই একটা Markdown table-এ দেখাবে।\n"
        ...
        "     | মান ১ | মান ২ |\n"
        "     ```\n"
        "   - প্রতিটা বিষয়ে অন্তত একটা প্রাসঙ্গিক table দেওয়ার চেষ্টা করো, যদি বিষয়টায় "
        "structured/সংখ্যাগত তথ্য থাকে।\n\n"
        ...
        "   - গাণিতিক সূত্র/সমীকরণ থাকলে সেটা কোড ব্লক বা স্পষ্টভাবে আলাদা লাইনে দেখাও এবং "
        "ধাপে ধাপে সমাধান করো।\n\n"
        "৪. ভাষা: সহজ, স্পষ্ট বাংলায় লিখবে (প্রয়োজনে ইংরেজি টার্ম রাখতে পারো)।\n\n"
        "৫. তুমি কোন AI বা কোন কোম্পানির তৈরি, সে সম্পর্কে কিছু বলবে না — নিজের নাম নিয়ে "
...
def split_markdown_for_telegram(text: str, max_len: int = 3500) -> list:
    ...
    লাইন-ধরে-ধরে ভাগ করা হয় — টেক্সট কখনো hard-truncate করা হয় না, পুরোটাই
    কোনো না কোনো chunk-এ থাকবে।
    """
    text = (text or "").strip()
    if not text:
        return [""]
    ...
    for block in blocks:
        ...
        # এই একটা block-ই max_len-এর চেয়ে বড় — লাইন ধরে ধরে ভাগ করো
        sub = ""
...
async def send_long_qa_answer(bot, chat_id: int, answer_text: str, reply_to_message_id: int = None):
    ...
    chunks = split_markdown_for_telegram(rich_text, max_len=3500)
    total = len(chunks)
...
async def send_solved_answer(bot, chat_id: int, status_msg, question: str, options: list,
    ...
    result_html = _result_to_html(result_text)
...
async def solve_poll(question: str, options: list, correct_idx: int = None) -> str:
    ...
    prompt = (
        ...
        f"প্রশ্ন: {question}\n\nঅপশন:\n{opts_text}{poll_hint}\n\n"
        "কঠোর নিয়ম: কোনো LaTeX ($), ** markdown ব্যবহার করবে না। "
        "সূত্র plain text-এ লিখবে। সহজ বাংলায় লিখবে।\n\n"
