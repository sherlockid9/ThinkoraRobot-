...
def _record_provider_result(stat_id: str, label: str, success: bool, error: str = "", limited: bool = False,
    ...
    if entry is None:
        entry = {"success": 0, "fail": 0, "last_error": "", "last_used": 0.0,
                  ...
                  "poll_success": 0, "text_success": 0, "poll_fail": 0, "text_fail": 0}
        provider_stats[stat_id] = entry
    task_key = "text" if task == "text" else "poll"
    if success:
        entry["success"] += 1
...
async def call_ai(prompt: str, task: str = "poll") -> str:
    """
    ...
    for round_num in range(max_rounds):
        ...
        while attempts < len(API_POOL):
            ...
            try:
                ...
                _record_provider_result(stat_id, label, success=False, error=f"HTTP {e.code}: {detail}", limited=(e.code == 429), task=task)
                if e.code in (401, 403):
                    _dead_keys.add(key)
                await _advance_active_provider(idx, label)
                if e.code == 429:
...
async def send_long_qa_answer(bot, chat_id: int, answer_text: str, reply_to_message_id: int = None):
    ...
    for i, piece in enumerate(chunks, start=1):
        ...
        if sent is None:
            fallback_piece = clean_text(piece)
            header = f"🤖 *{BOT_NAME}*\n\n" if i == 1 else ""
            try:
...
async def apistatus_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for p in API_POOL:
        ...
        if s:
            ...
            # প্রতিটি key দিয়ে কতটি poll solve আর কতটি text প্রশ্নের উত্তর হয়েছে
            txt_ok  = int(s.get("text_success", 0))
            poll_ok = int(s.get("poll_success", max(0, s["success"] - txt_ok)))
            total_poll_solves += poll_ok
            total_text_answers += txt_ok
            pool_lines += f"   \u21B3 \U0001F4CA Poll: <b>{poll_ok}</b> | Text Q&amp;A: <b>{txt_ok}</b>\n"
        else:
...
async def qa_retry_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid and uid != ADMIN_ID:
        ...
        try:
            await ctx.bot.send_message(chat_id, qa_daily_footer(uid).strip(), parse_mode="HTML")
        except Exception:
            pass
