...
# ══════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════
#  RICH MESSAGES (Telegram Bot API 10.1, June 2026) — sendRichMessage
#  একটা real, নতুন Bot API method যেটা "Rich Markdown" (GFM-ভিত্তিক) থেকে
#  আসল native heading, table (grid/border-সহ), bold/italic, এমনকি real
#  LaTeX render করে দেয় — legacy sendMessage parse_mode-এর মতো raw pipe/hash
#  character দেখায় না। python-telegram-bot library এখনো এটা wrap করে না,
#  তাই সরাসরি raw HTTPS POST দিয়ে call করা হচ্ছে (schema: chat_id +
#  rich_message: {"markdown": "..."} — core.telegram.org/bots/api#sendrichmessage)।
#  ব্যর্থ হলে (পুরনো client / সাময়িক network issue) None রিটার্ন করে, caller
#  তখন legacy plain sendMessage-এ fallback করে (দেখো send_long_qa_answer)।
# ══════════════════════════════════════════════════════════════════
...
def format_qa_rich_answer(answer_text: str) -> str:
    ...
    উত্তরের মূল, সবচেয়ে বড় title হিসেবে দেখা যায় (reference bot-গুলোর মতোই)। AI-এর output
    এমনিতেই GFM markdown (## heading, **bold**, | table |, $LaTeX$) — এগুলো এখানে ভাঙা হয়
    না, কারণ send_rich_message() (Telegram Rich Markdown) এসবই native table/heading/LaTeX
    হিসেবে render করে দেয়।"""
...
def _is_table_block(block: str) -> bool:
    ...
    split_markdown_for_telegram() এটা ব্যবহার করে table-কে মাঝপথে না কেটে chunk করতে।"""
...
async def send_long_qa_answer(bot, chat_id: int, answer_text: str, reply_to_message_id: int = None):
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        sent = await send_rich_message(chat_id, piece, reply_to_message_id=reply_id)
        if sent is None:
            # Rich Message ব্যর্থ হলে legacy plain Markdown-এ fallback — এই path-এই
            # শুধু delatex() চালানো হয় (LaTeX-কে Unicode-এ আনার জন্য) আর table/heading
            # legacy-Markdown-এ ভেঙে না যাওয়ার জন্য clean_text() দিয়ে সরল করা হয়,
            # কারণ legacy sendMessage কখনোই real table render করতে পারে না।
            fallback_piece = clean_text(delatex(chunk))
            if total > 1:
                fallback_piece += f"\n\n(অংশ {i}/{total})"
            header = f"🤖 *{BOT_NAME}*\n\n" if i == 1 else ""
            try:
                ...
                try:
                    await bot.send_message(chat_id, f"{header}{fallback_piece}", reply_to_message_id=reply_id)
                except Exception as e:
                    logger.error(f"send_long_qa_answer fallback failed (part {i}/{total}): {e}")
