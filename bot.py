...
def build_qa_prompt(question: str) -> str:
    ...
    return (
        ...
        "   - এই চ্যাটে LaTeX render হয় না। তাই কখনোই \\frac, \\sqrt, \\Delta, "
        "\\text{}, ^{}, _{}, $...$ ব্যবহার করবে না — লিখলে ভাঙা raw code দেখাবে।\n"
        "   - এর বদলে সরাসরি Unicode ব্যবহার করবে: ভগ্নাংশ a/b, সূচক x², xⁿ (জটিল হলে "
        "x^(n+1)), subscript T₀ Tₕ, বর্গমূল √(a²+b²), গ্রিক অক্ষর Δ δ α β θ π η μ λ ω Ω Σ ∞, "
        "চিহ্ন × ÷ ± ≤ ≥ ≠ ≈ ∝ ∫ →, একক normal টেক্সটে (500 J, 300 K)।\n"
...
def build_image_qa_prompt(question: str) -> str:
    ...
    return (
        ...
        "• এই চ্যাটে LaTeX render হয় না। কখনোই \\frac, \\sqrt, \\Delta, \\text{}, ^{}, _{}, "
        "$...$ ব্যবহার করবে না — বদলে Unicode লিখবে: a/b, x², xⁿ, T₀, √(a²+b²), Δ α β θ π, "
        "× ÷ ± ≤ ≥ ≠ ≈ →।\n"
...
async def apistatus_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for p in API_POOL:
        ...
        if s:
            ...
            # প্রতিটি key দিয়ে কতটি poll, কতটি text Q&A আর কতটি OCR (ছবি) solve হয়েছে
            ...
            total_poll_solves  += poll_ok
            total_text_answers += txt_ok
            total_ocr_answers  += ocr_ok
            pool_lines += (
                f"   \u21B3 \U0001F4DD Poll: <b>{poll_ok}</b>\u2705/{poll_bad}\u274c | "
                f"\U0001F4AC Text: <b>{txt_ok}</b>\u2705/{txt_bad}\u274c | "
                f"\U0001F5BC OCR: <b>{ocr_ok}</b>\u2705/{ocr_bad}\u274c\n"
            )
        else:
    ...
    usage_summary = (
        f"\n\U0001F4CA <b>Total Usage (all keys)</b>\n"
        f"\U0001F4DD Poll solved: <b>{total_poll_solves}</b>\n"
        f"\U0001F4AC Text Q&amp;A answered: <b>{total_text_answers}</b>\n"
        f"\U0001F5BC OCR (image) solved: <b>{total_ocr_answers}</b>\n"
        f"\U0001F464 Text limit/user/day: <b>{TEXT_DAILY_LIMIT}</b>\n"
        f"\U0001F464 OCR limit/user/day: <b>{OCR_DAILY_LIMIT}</b>\n"
    )
...
#  /dailyusage COMMAND
# ══════════════════════════════════════════════════════════════════
def _usage_bar(used: int, cap: int, slots: int = 10) -> str:
    """Rich progress bar — used/cap অনুযায়ী রঙ বদলায়।"""
    if cap <= 0:
        return "⬜" * slots
    ratio = min(used / cap, 1.0)
    fill  = round(ratio * slots)
    if ratio >= 1.0:
        block = "🟥"
    elif ratio >= 0.7:
        block = "🟧"
        ...
        block = "🟩"
    return block * fill + "▫️" * (slots - fill)


def _pct(used: int, cap: int) -> int:
    return int(round((min(used, cap) / cap) * 100)) if cap else 0


def _usage_row(name: str, used: int, cap: int) -> str:
    """Monospace table-এর একটি সারি — column গুলো fixed width."""
    left = max(0, cap - used)
    return (f"{name:<11}"
            f"{used:>4}"
            f"{left:>6}"
            f"{cap:>6}"
            f"{str(_pct(used, cap)) + '%':>6}")


async def dailyusage_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """📊 Daily Usage — student-এর poll / text Q&A / image (OCR) —
    তিনটার পূর্ণ বিস্তারিত limit report, Telegram-এর rich text
    (real monospace table + expandable blockquote) দিয়ে সাজানো।"""
    import html as _html
    ...
    name = _html.escape(user.first_name or "Student")

    # ── Reset ETA (Dhaka) ──────────────────────────────────────────
    dhaka_tz_r = pytz.timezone("Asia/Dhaka")
    now_dhaka  = datetime.now(dhaka_tz_r)
    midnight   = (now_dhaka + _dt.timedelta(days=1)).replace(
        hour=0, minute=0, second=0, microsecond=0)
    secs_left  = int((midnight - now_dhaka).total_seconds())
    reset_eta  = f"{secs_left // 3600}ঘ {(secs_left % 3600) // 60}মি"
    today_str  = now_dhaka.strftime("%d %b %Y")
    clock_str  = now_dhaka.strftime("%I:%M %p")

    # ── ADMIN ──────────────────────────────────────────────────────
    if user.id == ADMIN_ID:
        await update.message.reply_text(
            f"👑 <b>Admin Account</b>\n"
            f"<i>{today_str} • {clock_str} (Dhaka)</i>\n\n"
            f"<pre>Service      Used  Left   Cap\n"
            f"─────────────────────────────\n"
            f"Poll            ∞     ∞     ∞\n"
            f"Text Q&amp;A        ∞     ∞     ∞\n"
            f"Image OCR       ∞     ∞     ∞</pre>\n"
            f"🚀 <b>কোনো daily limit নেই — unlimited!</b>",
    ...
    if entry is None or entry.get("date") != today:
        ...
        poll_used = entry.get("count", 0)

    bonus       = get_referral_bonus(user.id)
    admin_extra = get_admin_extra_limit(user.id)
    poll_cap    = get_effective_daily_limit(user.id)
    poll_left   = max(0, poll_cap - poll_used)

    # ── Text Q&A usage ────────────────────────────────────────────
    text_used = get_qa_daily_entry(user.id)["count"]
    text_cap  = TEXT_DAILY_LIMIT
    text_left = max(0, text_cap - text_used)

    # ── Image (OCR) usage ─────────────────────────────────────────
    ocr_used = get_ocr_daily_entry(user.id)["count"]
    ocr_cap  = OCR_DAILY_LIMIT
    ocr_left = max(0, ocr_cap - ocr_used)

    total_used = poll_used + text_used + ocr_used
    total_cap  = poll_cap + text_cap + ocr_cap
    total_left = max(0, total_cap - total_used)

    # ── Cooldown ──────────────────────────────────────────────────
    last_time = entry.get("last_time", 0.0) if entry else 0.0
    elapsed   = time.time() - last_time
    if elapsed < COOLDOWN_SECS:
        cd   = int(COOLDOWN_SECS - elapsed) + 1
        cooldown_text = f"⏳ পরের poll: <b>{cd // 60}:{cd % 60:02d}</b> মিনিট পরে"
    else:
        cooldown_text = "✅ <b>এখনই solve করতে পারো!</b>"

    qa_last  = qa_rate_data.get(user.id, 0.0)
    qa_gap   = time.time() - qa_last
    if qa_gap < QA_COOLDOWN_SECS:
        cd2 = int(QA_COOLDOWN_SECS - qa_gap) + 1
        qa_cooldown_text = f"⏳ পরের প্রশ্ন: <b>{cd2 // 60}:{cd2 % 60:02d}</b> মিনিট পরে"
    else:
        qa_cooldown_text = "✅ <b>প্রশ্ন করতে পারো এখনই!</b>"

    # ── REAL TABLE (monospace <pre>) ──────────────────────────────
    table = (
        "Service     Used  Left   Cap  Use%\n"
        "──────────────────────────────────\n"
        + _usage_row("Poll",      poll_used, poll_cap) + "\n"
        + _usage_row("Text Q&A",  text_used, text_cap) + "\n"
        + _usage_row("Image OCR", ocr_used,  ocr_cap)  + "\n"
        "──────────────────────────────────\n"
        + _usage_row("TOTAL",     total_used, total_cap)
    )
    table = table.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")

    # ── Limit breakdown (expandable blockquote) ───────────────────
    breakdown = (
        f"📦 Base poll limit: <b>{DAILY_LIMIT}</b>\n"
        f"🎁 Referral bonus: <b>+{bonus}</b>\n"
        f"🛠 Admin extra: <b>+{admin_extra}</b>\n"
        f"🏆 আজকের মোট poll limit: <b>{poll_cap}</b>\n"
        f"✍️ Text Q&amp;A limit: <b>{text_cap}</b>/দিন\n"
        f"🖼 Image (OCR) limit: <b>{ocr_cap}</b>/দিন\n"
        f"⏱ Poll cooldown: <b>{COOLDOWN_SECS // 60} মিনিট</b>\n"
        f"⏱ Q&amp;A cooldown: <b>{QA_COOLDOWN_SECS // 60} মিনিট</b>\n"
        f"🕛 প্রতিদিন রাত ১২টায় (Dhaka) সব limit reset হয়।"
    )

    warn = ""
    if poll_left == 0 and text_left == 0 and ocr_left == 0:
        warn = "\n🚫 <b>আজকের সব limit শেষ! কাল আবার এসো।</b>"
    else:
        low = []
        if 0 < poll_left <= 3: low.append(f"poll {poll_left}টি")
        if 0 < text_left <= 3: low.append(f"প্রশ্ন {text_left}টি")
        if 0 < ocr_left  <= 3: low.append(f"ছবি {ocr_left}টি")
        if low:
            warn = "\n⚠️ <b>আর মাত্র " + " • ".join(low) + " বাকি!</b>"

    text = (
        f"📊 <b>DAILY USAGE REPORT</b>\n"
        f"👤 <b>{name}</b>\n"
        f"📅 <i>{today_str} • {clock_str} (Dhaka)</i>\n\n"
        f"<pre>{table}</pre>\n"
        f"🧩 <b>Poll Solve</b>  {poll_used}/{poll_cap}\n"
        f"{_usage_bar(poll_used, poll_cap)}\n\n"
        f"✍️ <b>Text Q&amp;A</b>  {text_used}/{text_cap}\n"
        f"{_usage_bar(text_used, text_cap)}\n\n"
        f"🖼 <b>Image (OCR)</b>  {ocr_used}/{ocr_cap}\n"
        f"{_usage_bar(ocr_used, ocr_cap)}\n\n"
        f"{cooldown_text}\n"
        f"{qa_cooldown_text}\n\n"
        f"<blockquote expandable>🔎 <b>বিস্তারিত limit breakdown</b>\n{breakdown}</blockquote>\n"
        f"🔄 <b>Reset হবে:</b> {reset_eta} পরে (রাত ১২টা)\n"
        f"🎯 <b>মোট বাকি আজ:</b> {total_left}/{total_cap}"
        f"{warn}"
    )
    ...
    share_keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Share & Get +Bonus Limit",
