...
def _render_frame(dot_i: int, stage_i: int) -> str:
    ...
    filled = round(((stage_i + 1) / total_stages) * 10)
    bar = "🟩" * filled + "⬜" * (10 - filled)
    return f"*{label}{dots}*\n\n`{bar}`"
...
def build_qa_prompt(question: str) -> str:
    return (
        ...
        "কঠোরভাবে মানবে:\n\n"
        "১. HEADING (সীমিত রাখবে — অত্যন্ত গুরুত্বপূর্ণ):\n"
        "   - মূল বিষয়ের title-এর জন্য `## ` (H2) একবার ব্যবহার করো — যেমন: `## আউফবাউ নীতি`\n"
        "   - এর নিচে সর্বোচ্চ ২-৩টা `### ` (H3) sub-section রাখবে (যেমন মূল সংজ্ঞা+নিয়ম, "
        "একটা উদাহরণ, দরকার হলে ব্যতিক্রম/গুরুত্ব) — প্রতিটা ছোট sub-topic-এর জন্য আলাদা "
        "heading বানিয়ে উত্তরকে ভেঙে ভেঙে লম্বা করবে না; কাছাকাছি বিষয়গুলো এক section-এই "
        "এক-দুই paragraph/bullet-এ একসাথে বলে দেবে।\n"
        "   - কোনো section title-কে শুধু **bold** করে হেডিং বানাবে না, `##`/`###` দিয়েই "
        "বানাবে। **bold** শুধু বাক্যের ভেতরে জরুরি শব্দ জোর দিতে ব্যবহার করবে।\n\n"
        "২. TABLE (প্রতিটা উত্তরে অন্তত ১টা table বাধ্যতামূলক — real GFM markdown table "
        "বানাতে হবে, শুধু লিখে দিলে হবে না):\n"
        "   - প্রতিটা উত্তরে (একদম simple হলেও) কমপক্ষে ১টা relevant table অবশ্যই দেবে — "
        "মূল তথ্য/সংজ্ঞা/পয়েন্টগুলো সংক্ষেপে সাজিয়ে (যেমন: বিষয় বনাম ব্যাখ্যা, ধাপ বনাম "
        "ফলাফল, নিয়ম বনাম উদাহরণ) — কারণ table student-দের জন্য সবচেয়ে সহজে পড়া যায়।\n"
        "   - প্রয়োজন হলে আরও ১-২টা table দেওয়া যাবে (যেমন সত্যিকারের একটা তুলনা আর "
        "আলাদা একটা উদাহরণ-তালিকা) — কিন্তু প্রতিটা ছোট sub-point-এর জন্য আলাদা table "
        "বানিয়ে উত্তরকে অযথা লম্বা করবে না; নিয়ম ৪-এর সংক্ষিপ্ততা মেনেই table(গুলো) ছোট ও "
        "কার্যকর রাখবে।\n"
        ...
        "দেবে না)। Math/সমস্যা সমাধানেও নিচের নিয়ম ৪ অনুযায়ী আগে **চূড়ান্ত উত্তর** ছোট করে "
        "বলে দেবে, তারপর নিচে সংক্ষেপে ধাপে ধাপে সমাধান দেখাবে (প্রতিটা ধাপ আলাদা লাইনে, "
        "কিন্তু অহেতুক লম্বা না করে)।\n\n"
        "৪. উত্তরের দৈর্ঘ্য ও গঠন (কঠোরভাবে মানতে হবে — সবচেয়ে গুরুত্বপূর্ণ নিয়ম):\n"
        ...
        "প্রয়োজনে ২-৩টা অংশে ভাগ করেও দেওয়া যাবে। সাধারণ/সহজ প্রশ্নের জন্য কখনোই এত বড় "
        "করবে না।\n"
        "   - ধারা (order): সবার প্রথমে প্রশ্নের সরাসরি মূল উত্তরটা ১-২ লাইনে স্পষ্টভাবে দেবে "
        "(যেমন: `✅ **উত্তর:** ...` অথবা numeric/formula প্রশ্নে সরাসরি final answer), "
        "তারপর তার নিচে সংক্ষেপে ব্যাখ্যা/কারণ/ধাপ দেখাবে — ব্যাখ্যাও যতটা সম্ভব সংক্ষিপ্ত ও "
        "সঠিকভাবে বুঝানোর মতো রাখবে, অযথা লম্বা করবে না।\n"
        "   - উত্তর যত বড়ই হোক (বড় প্রশ্নের ক্ষেত্রে), কোনো অবস্থাতেই মাঝ-বাক্যে/মাঝ-সূত্রে/"
        "মাঝ-table-এ থেমে যাওয়া চলবে না — শেষ পর্যন্ত সম্পূর্ণ করেই থামবে।\n\n"
