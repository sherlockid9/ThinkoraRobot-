...
def _render_frame(dot_i: int, stage_i: int) -> str:
    ...
    pct = round(((stage_i + 1) / total_stages) * 100)
    bar_len = 12
...
def build_qa_prompt(question: str) -> str:
    return (
        ...
        "উত্তর একটা পরিপাটি, ডকুমেন্টের মতো structured format-এ দিতে হবে — নিচের নিয়ম "
        "কঠোরভাবে মানবে:\n\n"
        "উত্তর অবশ্যই নিচের FIXED TEMPLATE অনুসরণ করে লিখতে হবে (প্রবাহ নামের একটা reference "
        "বটের answer style অনুযায়ী বানানো — ঠিক এই ধারা/সাজানো মেনে চলবে):\n\n"
        "```\n"
        "## <একটা relevant emoji> <বিষয়ের নাম>\n\n"
        "**সংজ্ঞা:** <১-২ লাইনে মূল সংজ্ঞা/ধারণা, জরুরি term **bold**>\n\n"
        "🔍 **<প্রসঙ্গ অনুযায়ী section নাম — যেমন প্রধান পার্থক্যসমূহ/মূল তথ্য/ধাপসমূহ>**\n"
        "<এখানে table>\n\n"
        "🧠 **মনে রাখার টিপস**\n"
        "<২-৩ লাইনের ছোট bullet — মনে রাখার সহজ কৌশল/mnemonic>\n\n"
        "✅ **Final Answer**\n"
        "<প্রশ্নের সরাসরি চূড়ান্ত উত্তর, ১-২ লাইনে, একদম শেষে>\n"
        "```\n\n"
        "১. HEADING (সীমিত রাখবে):\n"
        "   - উপরের template অনুযায়ী একটাই `## ` (H2) title (emoji-সহ) — যেমন: "
        "`## 🌱 নগ্নবীজি বনাম আবৃতবীজি`। এর নিচে আলাদা `###` heading না বানিয়ে "
        "**সংজ্ঞা:**, 🔍, 🧠, ✅ — এই bold/emoji label-গুলো দিয়েই section আলাদা করবে "
        "(ঠিক টেমপ্লেটে যেমন দেখানো আছে)।\n"
        "   - প্রতিটা ছোট sub-topic-এর জন্য আলাদা `###` heading বানিয়ে উত্তরকে ভেঙে ভেঙে "
        "লম্বা করবে না।\n\n"
        "২. TABLE (🔍 section-এ বাধ্যতামূলক — real GFM markdown table বানাতে হবে, শুধু "
        "লিখে দিলে হবে না):\n"
        "   - 🔍 section-এ কমপক্ষে ১টা relevant table অবশ্যই দেবে — তুলনা হলে দুই কলামে "
        "পাশাপাশি, না হলে মূল পয়েন্ট/ধাপ/উদাহরণ সংক্ষেপে সাজিয়ে।\n"
        "   - প্রয়োজন হলে আরেকটা ছোট table দেওয়া যাবে, কিন্তু প্রতিটা sub-point-এর জন্য "
        "আলাদা table বানিয়ে length বাড়াবে না — নিয়ম ৪-এর সংক্ষিপ্ততা মেনেই রাখবে।\n"
        "   - Table-এর সঠিক syntax (header row + separator row বাধ্যতামূলক):\n"
        "     ```\n"
        "     | কলাম ১ | কলাম ২ |\n"
        "     |---|---|\n"
        "     | মান ১ | মান ২ |\n"
        ...
        "দেবে না)। Math/সমস্যা সমাধানেও template অনুযায়ী 🔍 section-এ সংক্ষেপে ধাপে ধাপে "
        "সমাধান দেখাবে, আর ✅ Final Answer-এ একদম শেষে ছোট করে final numeric answer "
        "দেবে।\n\n"
        ...
        "প্রয়োজনে ২-৩টা অংশে ভাগ করেও দেওয়া যাবে — কিন্তু তখনও উপরের template-এর ধারা "
        "(সংজ্ঞা → 🔍 → 🧠 → ✅ Final Answer) বজায় রাখবে।\n"
        "   - উত্তর যত বড়ই হোক, কোনো অবস্থাতেই মাঝ-বাক্যে/মাঝ-সূত্রে/মাঝ-table-এ থেমে যাওয়া "
        "চলবে না — ✅ Final Answer পর্যন্ত সম্পূর্ণ করেই থামবে।\n\n"
