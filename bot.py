...
async def send_solved_answer(bot, chat_id: int, status_msg, question: str, options: list,
    ...
    combined = (
        f"🤖 <b>{_html_lib.escape(BOT_NAME)}</b>\n\n"
        f"<pre>{escaped_block}</pre>\n\n"
        f"{result_html}{footer_html}"
    )

    MAX = 4000
    if len(combined) > MAX:
        combined = combined[:MAX] + "\n\n<i>(বাকি অংশ কাটা হয়েছে)</i>"

    try:
        return await bot.send_message(chat_id, combined, parse_mode=ParseMode.HTML,
                                      disable_web_page_preview=True, reply_markup=reply_markup)
    except Exception as e:
        logger.warning(f"send_solved_answer HTML failed: {e}")
        # Fallback: এখনও code block-টা আলাদা করে হলেও পাঠানোর চেষ্টা করো
        try:
            await bot.send_message(chat_id, f"<pre>{escaped_block}</pre>", parse_mode=ParseMode.HTML)
        except Exception:
            try:
                await bot.send_message(chat_id, block_text)
            except Exception:
                pass
        try:
            return await bot.send_message(chat_id, clean_text(result_text),
                                          disable_web_page_preview=True, reply_markup=reply_markup)
        except Exception as e2:
            logger.error(f"send_solved_answer total failure: {e2}")
...
async def msg_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user and msg.text and msg.chat.type == "private":
        ...
        if question:
            ...
            rich_text = format_qa_rich_answer(answer)
            sent = await send_rich_message(msg.chat_id, rich_text, reply_to_message_id=msg.message_id)
            if sent is None:
                # পুরনো client / rich message পাঠাতে না পারলে সাধারণ Markdown-এ fallback
                fallback_text = clean_text(answer)
                await msg.reply_text(
                    f"🤖 *{BOT_NAME}*\n\n{fallback_text}",
                    parse_mode=ParseMode.MARKDOWN
                )
...
async def qa_retry_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    rich_text = format_qa_rich_answer(answer)
    sent = await send_rich_message(chat_id, rich_text)
    if sent is None:
        fallback_text = clean_text(answer)
        await ctx.bot.send_message(
            chat_id, f"🤖 *{BOT_NAME}*\n\n{fallback_text}", parse_mode=ParseMode.MARKDOWN
        )
